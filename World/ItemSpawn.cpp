...
void AItemSpawn::SpawnItem ()
{
	...
	{
		TArray<FlootableRow*>SpawnItems;
		LootTable->GetAllRows ( "", SpawnItems );
		const FlootableRow* LootRow = SpawnItems[FMath::RandRange ( 0, SpawnItems.Num () - 1 )];
		...
		if (LootRow && LootRow->Items.Num () && PickupClass)
		{
			...
			for (auto& ItemClass : LootRow->Items)
			{
				...
				FTransform SpawnTranform = GetActorTransform ();
				SpawnTranform.AddToTranslation ( LocationOffset );

				APickup* Pickup = GetWorld ()->SpawnActor<APickup> ( PickupClass, SpawnTranform, SpawnParams );
...
void AItemSpawn::OnItemTaken ( AActor* DestroyedActor )
{
	if (HasAuthority ())
	{
		...
		if (SpawnedPickups.Num () <= 0)
		{
			...
			GetWorldTimerManager ().SetTimer ( Timer_Handle_RespawnItem, this, &AItemSpawn::SpawnItem, FMath::RandRange ( RespawnRange.GetMin (), RespawnRange.GetMax ()), false  );
