...
//Called when the inventory is changed and the UI needs an update. Optional param for if an item changes.
...
{
	IAR_NoItemsAdd UMETA(DisplayName = "No items added"),
	IAR_SomeItemsAdded UMETA(DisplayName ="Some items added"),
	IAR_AllItemsAdded UMETA(DisplayName = "All item added")
};



//Represent the result of adding an item to the inventory
...
public:
	...
	//The amount of the item thas was actualy added to the end. Maybe we tried adding 10 items, but onlu 8 could be added because  of capacity/weight?
	UPROPERTY(BlueprintReadOnly, Category = "Item Add Result")
	...
	static FItemAddResult AddedNone(const int32 InItemQuantity, const FText& ErrorText)
	{
		...
		AddedNoneResult.Result = EItemAddResult::IAR_NoItemsAdd;
		AddedNoneResult.ErrorText = ErrorText;
		return AddedNoneResult;

	}

	static FItemAddResult AddedSome(const int32 IntItemQuantity,const int32 ActualAmountGiven, const FText& ErrorText)
	{
		FItemAddResult AddedSome(IntItemQuantity,ActualAmountGiven);
		AddedSome.Result = EItemAddResult::IAR_SomeItemsAdded;
		AddedSome.ErrorText = ErrorText;
		return AddedSome;
	}
	static FItemAddResult AddedAll(const int32 InItemQuantity)
	{
		FItemAddResult AddedAll(InItemQuantity);
		AddedAll.Result = EItemAddResult::IAR_AllItemsAdded;
		return AddedAll;
	}
...
public:	
	...
	* @return the amount of the item that was added to the inventory*/
	...
	/* Take some quantity away from the item, and remove it from inventory when quantity reaches zero.
	...
	//Get all inventory items thar are a child of ItemClass. Useful for grabbing all weapons, all food etc
	UFUNCTION(BlueprintPure, Category = "Inventory")
