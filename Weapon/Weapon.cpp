...
bool AWeapon::CanReload() const
{
	...
	return ((bCanReload == true) && (bGotAmmo == true) && (bStateOKToReload == true) && (!PawnOwner->IsAiming()));
}
...
{
	ASurvivalPlayerController* const PlayerController = PawnOwner->GetInstigator() ? Cast<ASurvivalPlayerController> (PawnOwner->GetInstigator()->Controller) : NULL;
	...
	else if (PawnOwner->GetInstigator())
	{
		FinalAim = PawnOwner->GetInstigator()->GetBaseAimRotation().Vector();
	}
...
FHitResult AWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace) const
{
	...
	FCollisionQueryParams TraceParams(SCENE_QUERY_STAT(WeaponTrace), true, PawnOwner->GetInstigator());
