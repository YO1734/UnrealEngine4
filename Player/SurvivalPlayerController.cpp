...
void ASurvivalPlayerController::ApplyRecoil ( const FVector2D& RecoilAmount, const float RecoilSpeed, const float RecoilResetSpeed, TSubclassOf<class UCameraShakeBase> Shake )
{
	if (IsLocalPlayerController ())
	{
		//if (PlayerCameraManager)
		//{
		//	PlayerCameraManager->PlayWorldCameraShake (GetWorld(), Shake,GetPawn()->GetActorLocation(), 1.f, 1, true);
		//}
...
void ASurvivalPlayerController::Turn ( float Rate )
{
	...
	if (!FMath::IsNearlyZero ( RecoilBumpAmount.X, 0.01f ))
	{
		...
		RecoilBumpAmount.X = FMath::FInterpTo ( RecoilBumpAmount.X, 0.f, GetWorld ()->GetDeltaSeconds (), CurrentRecoilSpeed );
	...
	RecoilResetAmount.X = FMath::FInterpTo ( RecoilResetAmount.X, 0.f, GetWorld ()->GetDeltaSeconds (), CurrentRecoilResetSpeed );
...
void ASurvivalPlayerController::LookUp ( float Rate )
{
	...
	{
		FVector2D LastCurrentRecoil = RecoilBumpAmount;
		RecoilBumpAmount.Y = FMath::FInterpTo ( RecoilBumpAmount.Y, 0.f, GetWorld ()->GetDeltaSeconds (), CurrentRecoilSpeed );
		AddPitchInput ( LastCurrentRecoil.Y - RecoilBumpAmount.Y );
	}

	//Slowly reset back to center after recoil is processed
	FVector2D LastRecoilResetAmount = RecoilResetAmount;
	RecoilResetAmount.Y = FMath::FInterpTo ( RecoilResetAmount.Y, 0.f, GetWorld ()->GetDeltaSeconds (), CurrentRecoilResetSpeed );
